...
<table style="width:100%; border-collapse: collapse; border: 0px solid black;">
  <tr style="background-color: #f1f1f1;">
    ...
    <th colspan="2"><p style="margin: 0 0 4px 0;">Re: Quotation</p>
    <p style="margin: 0 0 4px 0;">Dear {{$customer->name}},</p></th>
  </tr>
  <tr style="background-color: #f1f1f1;">
  ...
  @foreach($quote as $key => $product)
         <?php

        $productData = DB::table('products')->where('id',$product->product_id)->first();
        $productdescription = strip_tags($productData->description);
         if($product->quote_id == 0) {
         $productExtraData = DB::table('product_extra_info')->where('sales_orders_id',$product->id)->where('product_id',$product->product_id)->first();
         $productTradeData = DB::table('trade_ins')->where('sales_orders_id',$product->id)->where('old_product_id',$product->product_id)->first();
 
        }
        else{
         $productExtraData = DB::table('product_extra_info')->where('quote_id',$product->quote_id)->where('product_id',$product->product_id)->first();
 ...
 <tr style="background-color: #fe7e0f;">
  ...
  <tr>
    ...
    <td style=" padding: 8px;"> {{ $product->currency }}{{ $product->price }}</td>
...
<table style="width:100%; ">
  ...
  <tr  style="background-color: #f1f1f1;">
    ...
    <td style=" padding: 8px;">
     @if(!empty($productExtraData)) {{$productExtraData->hitch}}
     @endif
   </td>
    <td style=" padding: 8px;"> @if(!empty($productExtraData))
