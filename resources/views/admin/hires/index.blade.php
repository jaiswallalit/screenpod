...
<div class="content-wrapper">
   <div class="content-header">
      <div class="container-fluid">
         <div class="row mb-2">
            <div class="col-sm-6">
               <h1 class="m-0 text-dark">Hires</h1>
            </div>
            <div class="col-sm-6">
               <ol class="breadcrumb float-sm-right">
                  <li class="breadcrumb-item"><a href="#">Home</a></li>
                  <li class="breadcrumb-item active">Hire List</li>
               </ol>
            </div>
            </div>
         </div>
                  ...
                  <div class="card">
                     @if(!empty($data['checkRole']) && (Auth::user()->user_type == 'admin' || !empty($data['checkPermission'])))
                        <div class="card-header float-right">
                           <a href="{{route('hires.add')}}" class="btn btn-info float-right"><i class="fas fa-plus"></i> {{$data['actionData']->action_title}} </a>
                        </div>
                     @endif
                     <div class="card-body">
                              ...
                              <tr class="row1" id="row1" data-id="{{ $result->id }}">
                                 ...
                                 <td>
                                    @if(!empty($image->image))
                                       <ul class="list-inline"><li class="list-inline-item"><img style="width: 100px"; alt="Avatar" class="table-avatar" src="{{url('/public/admin/clip-one/assets/hires/thumbnail')}}/{{$image->image}}"></li></ul>
                                    @else
                                 ...
                                 <td>
                                    @foreach ($action_ids as $key1 => $action_id) 
                                       ...
                                       @if ($action->action_slug == 'edit' || $action->action_slug == 'delete' || $action->action_slug == 'view')
                                          <a href="{{route('hires.'.$action->action_slug,$result->id)}}" class="btn btn-{{$action->class}} btn-sm" data-placement="top" data-original-title="{{$action->action_title}}" style="width: 83px;" ><i class="{{$action->icon}}"></i>{{$action->action_title}}</a>&nbsp;
                                       @endif
                                    @endforeach
                                       ...
                                       $(function () {
                                          ...
                                          function sendOrderToServer() {  
                                             ...
                                             $.ajax({
                                                type: "POST", 
                                                dataType: "json", 
                                                url: "{{url('admin/hires/sortProducts')}}",
                                                data: {
                                                   order:order,
                                                   _token: '{{csrf_token()}}'
                                                },
                        ...
                        <div class="modal fade" id="modal-default">
                           <div class="modal-dialog">
                              <div class="modal-content">
                                 ...
                                 <form id="quickForm" action="{{route('hires.add_more')}}" method="POST" enctype="multipart/form-data" >
                                    {{csrf_field()}}
                                    <input type="hidden" name="id" id="hire_id" value="">
