...
class HireQuoteController extends Controller
{
    /*Create New HireQuote with old lead*/
    public function createHireQuoteWithExistingLead(Request $request)
    {
    ...
    /*Create New HireQuote with new lead*/
    public function createHireQuoteWithNewLead(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'user_id' => 'required',
            'name' => 'required',
            ...
            if (empty($lead_exist)) {
                ...
                if ($data->save()) {
                    ...
                    return response()->json(array(
                        'status' => 200,
                        'message' => 'Success',
                        'success_message' => '1Hire Quote created successfully.',
                        'data' => $data,
                    ), 200);
                }
            }
                ...
                ->where('customer_id',$request->customer_id)->where('status','Hire')
                ->first();
                 if (empty($quote_id)) {
                    $data = new Quote;
                    ...
                    if ($data->save()) {
                        ...
                        return response()->json(array(
                            'status' => 200,
                            'message' => 'Success',
                            'success_message' => '2 Hire Quote created successfully.',
                            'data' => $data,
                        ), 200);
                    }
                ...
                $data= array('lead_id'=>$lead_exist->id,'id'=>$quote_id->id,'customer_id'=>$request->customer_id,'date'=>date('Y-m-d'));
        
                return response()->json(array(
                    'status' => 200,
                    'message' => 'Success',
                    'success_message' => '3 Hire created successfully.',
                    'data' => $data,
                ), 200);
            } 
        }
...
public function saveDealers(Request $request)
{
    ...
    {
        $products = QuoteProduct::where('quote_id',$id)->where('sendprivew',1)->where('status','Hire')->get();
        ...
        foreach($products as $key => $product){
            ...
            $data[] = [
                ...
                'hire_info' => $this->getHireinfo($product->product_id,$id),
  ...
  public function getTradeIn($product_id,$quote_id)
    {
     ...
     /*Extrainfo Details*/
         public function getExtra($product_id,$quote_id)
          ...
          /*Hireinfo Details*/
         public function getHireinfo($product_id,$quote_id)
         {
             $hireinfo = HireInfo::where('product_id',$product_id)->where('quote_id',$quote_id)->get();
             $data = [];
             foreach($hireinfo as $key => $product){
               $productinfoData = DB::table('hire_info')->where('product_id',$product->product_id)->where('quote_id',$quote_id)->first();
                 $data[] = [
                    'min_hire_period' => $productinfoData->min_hire_period,
                    'payment_terms' => $productinfoData->payment_terms,
                    'purcharse_period' => $productinfoData->purcharse_period,
                    'consumables' => $productinfoData->consumables,
                    'transport_in' => $productinfoData->transport_in,
                    'weekly_hire_price' => $productinfoData->weekly_hire_price,
                    'fittings_price	' => $productinfoData->fittings_price,
                    'transport_out_price' => $productinfoData->transport_out_price,
                    'delivery_location' => $productinfoData->delivery_location,
                    'site_contact	' => $productinfoData->site_contact,
                    'hire_start' => $productinfoData->hire_start,
                    'hire_end' => $productinfoData->hire_end,
    ...
    public function quoteDetails(Request $request)
    {
        ...
        ->leftJoin("contracts", "contracts.quote_id", "=", "quotes.id")
                        ->select('quotes.*','leads.name','leads.email','leads.phone','leads.address',
                        'leads.user_id','leads.date','leads.status','contracts.agreement_no')
        ...
        $data[] = [
                ...
                'sent' => $quote->sent,
                'sent_on' => $quote->sent_on,
                'agreement_no' => $quote->agreement_no,
            ];
   ...
   /*Privew quote send Quotes   this are submite form */  
    ...
    public function sendPrivewHireQuote(Request $request)
    {
        ...
        if ($validator->fails()) { 
            ...
            $products = QuoteProduct::where('quote_id',$request->quote_id)->where('status','Hire')->where('sendprivew',1)->get();
            $contracts = Contract::where('quote_id',$request->quote_id)->get();
...
// die();
            if (!empty($data)) {
                ...
                //     )); 
                    ...
                    $file = "contracts_$request->quote_id.pdf";
                    $data->save();
  
                        $pdf = PDF::loadView('admin.hirequotes.hirepdf', $emailData);
                        Storage::disk('uploads')->put('contracts_' . $request->quote_id . '.pdf', $pdf->output());
                        DB::table('quotes')->where('id', $request->quote_id)
                            ->update(array(
                                'pdf_url' => $file,
                            )); 
...
//filter privew send quote screen//
    ...
    public function hiresendQuote(Request $request)
    {
...
// print_r($data);die;
 ...
 if(!empty($data->attachment)){
   ...
   public function inProgressHireQuote(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'user_id' => 'required',
        ]);
...
// die;
          ...
          return response()->json(array(
                                        'status' => 200,
                                        'message'=> 'Success',
                                        'success_message'=>'Get In Progress HireQuote.',
                                        'data' => $data,
                                    ),200);
    }
    ...
    public function filterInProgressHireQuote(Request $request)
    {
        ...
        ->where('leads.customer_id',$request->customer_id)
        ->where('leads.status','In Progress')
        ->where('quote_products.status','Hire')
   ...
   /* Hire Information */
public function HireInformation(Request $request)
{
    try {
            $validator = Validator::make($request->all(), [
            'quote_id' => 'required',
        ...
        if ($validator->fails()) {
            ...
            if($request->quote_id == 0){
                $hireexist = DB::table('hire_info')->where('hire_orders_id',$request->hire_orders_id)->where('product_id',$request->product_id)->first();
            
            $data= [];
            if (empty($hireexist)) {
                $info = new HireInfo;
                $info->hire_orders_id = $request->hire_orders_id;
                ...
                return response()->json(array(
                    'status' => 200,
                    'message'=> 'Success',
                    'success_message'=>'Hire Information Inserted successfully.',
                    
                    'data' => $info,
                ),200); 
            } 
            else {
                DB::table('hire_info')->where('quote_id', $request->quote_id)->where('product_id',$request->product_id)
                ->update(array(
                'min_hire_period' => $request->min_hire_period,
                'payment_terms' => $request->payment_terms,
                'purcharse_period' => $request->purcharse_period,
                'consumables' => $request->consumables,
                'transport_in' => $request->transport_in,
                'weekly_hire_price' => $request->weekly_hire_price,
                'fittings_price' => $request->fittings_price,
                'transport_out_price' => $request->transport_out_price,
                'delivery_location' => $request->delivery_location,
                'site_contact' => $request->site_contact,
                'hire_start' => $request->hire_start,
                'hire_end'	 => $request->hire_end,
               
            ));   return response()->json(array(
                    'status' => 200,
                    'message'=> 'Success',
                    'success_message'=>'Hire Information  Update successfully.',
                    'data' => $data,
                ),200);
            }
        }else{
            $hireexist = DB::table('hire_info')->where('quote_id',$request->quote_id)->where('product_id',$request->product_id)->first();
            $data= [];
            if (empty($hireexist)) {
                $info = new HireInfo;
...
public function contracts(Request $request)
    {
        try {
            ...
            if ($validator->fails()) {
				...
				$idtrd = DB::table('contracts')->where('quote_id',$request->quote_id)->first();
                $data= [];
                if (empty($idtrd)) {
                    $info = new Contract;
                    $info->quote_id = $request->quote_id;
                    $info->agreement_no = $request->agreement_no;
                    $info->offer_date = $request->offer_date;
                    ...
                    return response()->json(array(
                        'status' => 200,
                        'message'=> 'Success',
                        'success_message'=>'Contract in Inserted successfully.',
                        'contracts' => $data->pdf_url,
                        'data' => $info,
                    ),200); 
                } 
                else {
                    DB::table('contracts')->where('quote_id', $request->quote_id)
                    ->update(array(
                    ...
                    'telephone' => $request->telephone,
                        ...
                        'status' => 200,
                        'message'=> 'Success',
                        'success_message'=>'Contract  Update successfully.',
                        'data' => $data,
                    ),200);
                }
    ...
    public function prvquoteHireDetails(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'quote_id' => 'required',
        ]);
        if ($validator->fails()) { 
            ...
            $quote = Quote::join('leads','quotes.lead_id','=','leads.id')
                        ...
                        'users.name as user_name','users.email as user_email','users.mobile as user_mobile',
                        'customers.name as customer_name','customers.email as customer_email',
                        'contracts.name as contracts_name','contracts.agreement_no as contracts_agreement_no',
                        'contracts.offer_date as contracts_offer_date','contracts.company_name as contracts_company_name'
                        ,'contracts.telephone as contracts_telephone')
        ...
        $data[] = [
                ...
                'sent' => $quote->sent,
                'sent_on' => $quote->sent_on,
                'contracts_name' => $quote->contracts_name,
                'contracts_agreement_no' => $quote->contracts_agreement_no,
                'contracts_offer_date' => $quote->contracts_offer_date,
                'contracts_company_name' => $quote->contracts_company_name,
                'contracts_telephone' => $quote->contracts_telephone,
