...
class CustomerController extends Controller
{
	...
	public function savecustomeronlead(Request $request)
	{
		try {
			$validator = Validator::make($request->all(), [
				'name' => 'required',
	...
	public function update(Request $request)
	{
		try {
			...
			if ($validator->fails()) { 
                ...
                $lead_exist = DB::select('select * from leads where customer_id=?', [$request->id]);
                if($lead_exist){
		            DB::table('leads')
	      	...
	      	for ($i = 0; $i < count($productArr); $i ++){
		        if (array_diff_key($tableHeader,$productArr[$i])) {
		          ...
		          $data[] = [
		            ...
		            'address2' => $productArr[$i]['address2'],
		            'town' => $productArr[$i]['town'],
		            'county' => $productArr[$i]['county'],
		            'eircode' => $productArr[$i]['eircode']
		          ];
		        }
	      	}
