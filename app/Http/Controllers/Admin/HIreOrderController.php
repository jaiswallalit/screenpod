...
// die();
        if (!empty($request->from) || !empty($request->to) || !empty($request->serial_number) || !empty($request->customer) || !empty($request->user) || !empty($request->status) || $request->delivered == '0' || $request->delivered == '1' || !empty($request->dealer_id) || !empty($request->model)) {
            $query = HireOrder::join('products','hire_orders.product_id','=','products.id')
                                ->orderBy('id','DESC')
                                ->select('hire_orders.*','products.dealer_id','products.model');
            ...
            $data['results'] = HireOrder::orderBy('id','DESC')->paginate(1000);
        }
		return view('admin/hire_order/index',$data);
	}
     ...
     public function createSalesOrder()
            {
                ...
                return view('admin/hire_order/add',$data);
        ...
        public function save(Request $request)
        {
            try {
                ...
                if ($validator->fails()) { 
                    return redirect('admin/hire_order/add')
                    ...
                    $data->date_duration = $request->date_duration;
    ...
    public function view($id)
    {
		 ...
		 $sales_order = DB::table('hire_orders')->where('id',$id)->first(); 
      
        if ($sales_order->quote_id != 0) {
        
            $data['result'] = HireOrder::join('quotes','hire_orders.quote_id','=','quotes.id')
            ->join('leads','leads.id','=','quotes.lead_id')
            ->join('users','users.id','=','leads.user_id')
            ->leftjoin('customers','customers.id','=','hire_orders.customer_id')
            ->select('hire_orders.*','quotes.lead_id','quotes.attachment','customers.address as customers_address',
            ...
            ->where('hire_orders.id',$id)
            ->first();
        }
        else{
            // print_r('helloelef');
            // die();
            $data['result'] = HireOrder::select('hire_orders.*','customers.name as lead_name','customers.address as customers_address',
           ...
           ->leftjoin('customers','customers.id','=','hire_orders.customer_id')
           ->leftjoin('products','products.id','=','hire_orders.product_id')
            ->where('hire_orders.id',$id)
            ->first();
      }
    //   print_r($data['result']);
    //   die();
    	...
    	return view('admin.hire_order.view',$data);
    }
    ...
    {
    	$data = HireOrder::find($request->id);
        ...
        if ($request->type != 'all') {
            ...
            //$data->sub_total_price = ($request->price + $request->transport_price + $request->delivery_price) * $request->qty;
            //$data->sub_total_price =
            //End new entry

            $data->tax = ($request->price + $request->transport_price * $data->qty) * 23 / 100;
            $data->total_price = ($request->price + $request->transport_price * $data->qty) + $data->tax;
...
//==================================================
	...
	{
        $statements = DB::table('statements')->where('hire_orders_id',$id)->get();

    $hire_info = DB::table('hire_orders')->where('id',$id)->get();
    if(!empty($hire_info[0]->quote_id)){
  
    $data['machin'] = DB::table('hire_info')->where('quote_id',$hire_info[0]->quote_id)->where('product_id',$hire_info[0]->product_id)->get();
     }else{
 
    $data['machin'] = DB::table('hire_info')->where('hire_orders_id',$hire_info[0]->id)->where('product_id',$hire_info[0]->product_id)->get();
}
                $data['machin'] = DB::table('hire_info')->where('quote_id',$hire_info[0]->quote_id)->where('product_id',$hire_info[0]->product_id)->get();
                $data['result'] = HireOrder::select('hire_orders.*','customers.name as lead_name','customers.address as customers_address',
                'users.name as user_name','users.id as user_id','users.email as email','users.mobile as phone')
               ...
               ->where('hire_orders.id',$id)
                ->get();
                $data['user'] = User::find($hire_info[0]->user_id);
                $data['customer'] = Customer::find($hire_info[0]->customer_id);

            return view('admin.hire_order.contracts',$data);
	}
    //---------------------------------------------------------------
    ...
    {               
            ...
            try {
                if (empty($request->min_hire_period) && empty($request->payment_terms) && empty($request->purcharse_period) && empty($request->consumables) && empty($request->transport_in) && empty($request->weekly_hire_price) && empty($request->fittings_price) && empty($request->transport_out_price) && empty($request->delivery_location) && empty($request->site_contact) && empty($request->hire_start) && empty($request->hire_end)) {
                    session()->flash('message', 'Please enter any one value!');
                    Session::flash('alert-type', 'error');
                       return redirect('admin/hire_order/contracts'.'/'.$request->quote_id);
                }
                if($request->quote_id == 0){
                                         $product_exist = HireInfo::where('hire_orders_id',$request->hire_orders_id)->where('product_id',$request->product_id)->first();
                                     }else{
                                         $product_exist = HireInfo::where('quote_id',$request->quote_id)->where('product_id',$request->product_id)->first();
                                     }
     ...
     if (!empty($product_exist)) {
       ...
       ->update(array( 
        ...
        DB::table('hire_info')->insert(
            array(
                'quote_id' =>  $request->quote_id,
                'hire_orders_id' =>  $request->hire_orders_id,
                'product_id' => $request->product_id,
                'min_hire_period' =>  $request->min_hire_period,
                'payment_terms' => $request->payment_terms,
                'purcharse_period' => $request->purcharse_period,
    ...
    {               
            ...
            try {
                ...
                if ($validator->fails()) { 
                    return redirect('admin/hire_order/contract'.'/'.$request->hire_orders_id)
                                ->withErrors($validator)
                                ->withInput();
                } else {			        
                    $data = new Statement;
                    ...
                    //=========================================================
                    $data->hire_orders_id = $request->hire_orders_id;
