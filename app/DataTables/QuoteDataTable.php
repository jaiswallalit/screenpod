...
class QuoteDataTable extends DataTable
{
    ...
    public function query(Quote $model)
    {
        ...
        if (!empty($d['from']) || !empty($d['to']) || !empty($d['customer']) || !empty($d['status'])  || !empty($d['user']) || !empty($d['dealer']) || !empty($d['machine'])) {
            ...
            if (!empty($d['status'])) {
                //$query = $query->where('quotes.lead_id',$d['leads.user_id']);
                $query = $query->where('quotes.status','Sales');
            }
            if (!empty($d['machine'])) {
                $query = $query->whereRaw("find_in_set('".$d['machine']."',quote_products.product_id)");
            }
                        
            $data = $query->orderBy('id','desc')->select('quotes.*','leads.name as lead_name','users.name as sale_rep','leads.title as lead_title')->where('quotes.status','Sales');

        }else{
            $data = Quote::join('leads','leads.id','=','quotes.lead_id')
                    ->join('users','users.id','=','leads.user_id')
                    ->orderBy('id','desc')
                    ->select('quotes.*','leads.name as lead_name','users.name as sale_rep','leads.title as lead_title')->where('quotes.status','Sales');
        }
