...
class QuoteReportDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->editColumn('status', function($row) {
                if($row->status == '1'){
                    return '<a href="" class="btn btn-secondary btn-sm" onclick="return confirm("Are you sure want to change status?")">"Quote sent"</a>';
                }else{
                    return '<span href="" class="btn btn-success btn-sm" onclick="return confirm("Are you sure want to change status?")">Quote sent</span>';
                }
            })
    ...
    public function query(Quote $model)
    {
        ...
        if (!empty($d['from']) || !empty($d['to']) || !empty($d['customer'])|| !empty($d['sent'])  || !empty($d['user']) || !empty($d['dealer']) || !empty($d['machine'])) {
            ...
            if (!empty($d['sent'])) {
                //$query = $query->where('quotes.lead_id',$d['leads.user_id']);
                $query = $query->where('quotes.sent',$d['sent']);
            }
