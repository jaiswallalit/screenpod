...
class HireQuoteDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->editColumn('date', function($row) {
                $date_time = date('d F Y',strtotime($row->date));
                return $date_time;
            })
                ...
                foreach ($action_ids as $key => $action_id) {
                    ...
                    if (!empty($checkPermission) || Auth::user()->user_type == 'admin') {
                        if ($action->action_slug == 'edit' || $action->action_slug == 'delete' || $action->action_slug == 'view' || $action->action_slug == 'password') {
                            $btn .= '<a href="'.route("hirequotes.$action->action_slug",$row->id).'" class="btn btn-'.$action->class.' btn-sm" data-placement="top" data-original-title="'.$action->action_title.'" onclick="return confirm("Are you sure?")"><i class="'.$action->icon.'"></i>'.$action->action_title.'</a>&nbsp;';
                        }
                    }
                }
                return $btn;
    ...
    public function query(Quote $model)
    {
        ...
        if (!empty($d['from']) || !empty($d['to']) || !empty($d['customer']) || !empty($d['status'])  || !empty($d['user']) || !empty($d['dealer']) || !empty($d['machine'])) {
            ...
            if (!empty($d['user'])) {
                //$query = $query->where('quotes.lead_id',$d['leads.user_id']);
                $query = $query->whereRaw("find_in_set('".$d['user']."',leads.user_id)");
            }
            if (!empty($d['dealer'])) {
                //$query = $query->where('quotes.lead_id',$d['leads.user_id']);
                $query = $query->whereRaw("find_in_set('".$d['dealer']."',products.dealer_id)");
            }
            if (!empty($d['status'])) {
                //$query = $query->where('quotes.lead_id',$d['leads.user_id']);
                $query = $query->where('quotes.status','Hire');
            }
            if (!empty($d['machine'])) {
                $query = $query->whereRaw("find_in_set('".$d['machine']."',quote_products.product_id)");
            }
                        
            $data = $query->orderBy('id','desc')->select('quotes.*','leads.name as lead_name','users.name as sale_rep','leads.title as customer')->where('quotes.status','Hire');

        }else{
            $data = Quote::join('leads','leads.id','=','quotes.lead_id')
                    ->join('users','users.id','=','leads.user_id')
                    ->orderBy('id','desc')
                    ->select('quotes.*','leads.name as lead_name','users.name as sale_rep','leads.title as customer')->where('quotes.status','Hire');
        }
