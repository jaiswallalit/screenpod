...
class QuoteDataTable extends DataTable
{
            ...
            ->editColumn('action', function($row) use ($roles) {
                ...
                foreach ($action_ids as $key => $action_id) {
                    ...
                    if (!empty($checkPermission) || Auth::user()->user_type == 'admin') {
                        if ($action->action_slug == 'edit' || $action->action_slug == 'delete' || $action->action_slug == 'view' ||$action->action_slug == 'addtrade' || $action->action_slug == 'password') {
    ...
    public function query(Quote $model)
    {
        ...
        if (!empty($d['from']) || !empty($d['to']) || !empty($d['customer'])  || !empty($d['user']) || !empty($d['dealer']) || !empty($d['machine'])) {
